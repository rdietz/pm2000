...
// Get EEPROM values
  ...
  if (flag != 123)
  {
    ...
    RELAY3_DELAY = 500;
    EEPROM.put(AddrRelay3Delay, RELAY3_DELAY);
    RELAY3_PULSE_TIME = 500;
    EEPROM.put(AddrRelay3Time, RELAY3_PULSE_TIME);

    RELAY4_DELAY = 500;
    EEPROM.put(AddrRelay4Delay, RELAY4_DELAY);
    RELAY4_PULSE_TIME = 500;
    EEPROM.put(AddrRelay4Time, RELAY4_PULSE_TIME);
    delay(500);
    ...
    EEPROM.get(AddrRelay3Time, RELAY3_PULSE_TIME);
    EEPROM.get(AddrRelay3Delay, RELAY3_DELAY);
    Serial.print(F("Initialised Relay3 values: ")); Serial.print(RELAY3_PULSE_TIME);Serial.print(" - "); Serial.println(RELAY3_DELAY);
    
    EEPROM.get(AddrRelay4Time, RELAY4_PULSE_TIME);
    EEPROM.get(AddrRelay4Delay, RELAY4_DELAY);
    Serial.print(F("Initialised Relay4 values: ")); Serial.print(RELAY4_PULSE_TIME);Serial.print(" - "); Serial.println(RELAY4_DELAY);
...
// Get stored params
  ...
  Serial.println(RELAY3_DELAY);
  Serial.println(RELAY3_PULSE_TIME);
  Serial.println(RELAY4_DELAY);
  Serial.println(RELAY4_PULSE_TIME);
...
void loop()
{
  ...
  int incomingByte = 0;   // for incoming serial data
    ...
    if (mode == CALMODE)
    {
      if (millis() - calibrateMillis >= calibrateTime)
      {
		...
		{
			relay1.Trigger(Pulse2Trigger); // Start relay1 timing and register relay2 callback function to daisychain ////////////////////////////////
			counter = 0;
			lastCounter = 0;
  ...
  relay3.Update(RELAY3_DELAY, RELAY3_PULSE_TIME); // process any relay 3 activities
  relay4.Update(RELAY4_DELAY, RELAY4_PULSE_TIME); // process any relay 4 activities
} // End loop()
...
void Pulse2Trigger()
{
	relay2.Trigger(Pulse3Trigger);   // Start relay2 timing and register relay3 callback function to daisychain
  Serial.print(F("\nWait ")); Serial.print(RELAY2_DELAY); Serial.println(F(" mSecs"));
  Serial.println(F("Operate Relay 2")); 
}

void Pulse3Trigger()
{
  relay3.Trigger(Pulse4Trigger);   // Start relay3 timing and register relay4 callback function to daisychain
  Serial.print(F("\nWait ")); Serial.print(RELAY3_DELAY); Serial.println(F(" mSecs"));
  Serial.println(F("Operate Relay 3")); 
}

void Pulse4Trigger()              // Start relay4 timing 
{
  relay4.Trigger();
  Serial.print(F("\nWait ")); Serial.print(RELAY4_DELAY); Serial.println(F(" mSecs"));
  Serial.println(F("Operate Relay 4\n")); 
}
